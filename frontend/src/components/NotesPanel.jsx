  ...
  return (
    ...
    >
      ...
      <div className="bg-[#5C5C5C] border-b-2 border-[#0F0F0F] px-4 py-3 flex items-center justify-between">
        <div>
          <h3 className="font-pixel text-2xl text-[#0F0F0F] leading-none">
            {petName}'s desk
          </h3>
          <p className="text-xs text-[#0F0F0F]/70 mt-1">notes &amp; little reminders</p>
        </div>
        <button
      ...
      <div className="px-4 py-3 flex-1 overflow-y-auto cozy-scroll">
        {tab === "reminders" && (
          <ul data-testid="reminders-list" className="space-y-2">
            {reminders.length === 0 && (
              <li className="text-center text-sm text-[#6E6E6E] py-6">
                no reminders yet — try{" "}
                <span className="font-pixel text-base text-[#0F0F0F]">
                  "standup in 15 min"
                </span>
              </li>
            )}
            {reminders.map((r) => (
              <li
                ...
                className={`bg-white border-2 border-[#0F0F0F] rounded-lg p-3 ${
                  r.fired ? "opacity-60" : ""
                }`}
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-[#0F0F0F] break-words">
                      {r.content}
                    </p>
                    <div className="flex items-center gap-1.5 mt-1.5">
                      ...
                      {r.fired && (
                        <span className="font-pixel text-[10px] text-[#6E6E6E] uppercase">
                          done
                        </span>
                      )}
                    </div>
          ...
          <ul data-testid="notes-list" className="space-y-2">
            ...
            {notes.map((n) => (
              <li
                ...
                className="bg-white border-2 border-[#0F0F0F] rounded-lg p-3 flex items-start justify-between gap-2"
              >
                <p className="text-sm text-[#0F0F0F] break-words flex-1 whitespace-pre-wrap">
                  {n.text}
                </p>
                <button
      ...
      <div className="bg-[#D2CDBC] border-t-2 border-[#0F0F0F] px-3 py-3">
        ...
        <p className="font-pixel text-[10px] text-[#6E6E6E] mt-2 text-center tracking-wider">
